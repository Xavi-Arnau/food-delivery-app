...
const Food = ({ isCartVisible, refFood }) => {
  ...
  const getTotalAmount = () => {
    ...
    for (const key in cartItems) {
      if (cartItems[key] > 0) {
        let itemInfo = products.find((products) => products.id === Number(key));
  ...
  return (
    <div
      className="my-12 max-w-[1600px] place-items-center lg:mx-auto flex flex-col gap-12"
      id="food"
      ref={refFood}
    >
      {isCartVisible && (
          ...
          {products.map((product) => {
            if (cartItems[product.id] !== 0) {
              return (
                <div
                  key={product.id}
                  className="relative glass my-3 grid grid-cols-5 overflow-hidden"
                >
                  <img
                    alt=""
                  ...
                  <div>
                    ...
                    <p className="product-details flex items-center space-x-4 ">
                      {product.productName}
                    </p>
      ...
      <div className="grid lg:grid-cols-4 place-items-center gap-6">
        {products.map((product) => (
          <div
            key={product.id}
            className="w-[380px] p-5 bg-white rounded-lg glass transition-all duration-200 hover:scale-110"
          >
            <img
            ...
            <div className="flex- flex-row justify-between items-center mt-5 gap-3">
              ...
              <button
                ...
                className="p-3 text-2xl w-14 rounded-xl bg-orange-400 hover:bg-orange-800"
              >
                <BsCart className="mx-auto" />
              </button>
              {cartItems[product.id] > 0 && (
                <div className="absolute flex items-center justify-center top-5 left-5 bg-green-600 font-bold rounded-lg h-12 w-12">
